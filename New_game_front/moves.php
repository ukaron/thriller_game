		...
		switch ($_POST['move']) {
				...
				if (file_exists("./coords.csv")) {
					...
					foreach ($data as $elem) {
						if (strpos($elem, "Firestorm") === 0) {
							$coords = explode(':', $elem);
							file_put_contents("./coords.csv", "Firestorm".':'.$coords[1].':'.($coords[2] + 10).PHP_EOL, FILE_APPEND);
			...
			case ('down'):
				...
				foreach ($data as $elem) {
					if (strpos($elem, "Firestorm") === 0) {
						$coords = explode(':', $elem);
						file_put_contents("./coords.csv", "Firestorm".':'.($coords[1] + 10).':'.($coords[2]).PHP_EOL, FILE_APPEND);
